...
class LabState extends State<Lab> {
  ...
  Future _saveData() async {
    ...
    try {
      formData.labDate = DateTime.now().toString();
      await authBloc.setLABData(formData);
      _showMessage("Lab report saved successfully.", "success");
    } catch (e) {
            ...
            child: Padding(
              ...
              child: Form(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("Diagnostic Investigation Details", style: cHeaderText.copyWith(fontSize: 18)),
                    const SizedBox(height: 24),
                    TextFormField(
                      decoration: const InputDecoration(labelText: "Referring Provider/Dept", prefixIcon: Icon(Icons.business_rounded)),
                      onChanged: (v) => formData.from = v,
                      validator: evalName,
                    ),
                    const SizedBox(height: 16),
                    TextFormField(
                      decoration: const InputDecoration(labelText: "Pathology Tests", prefixIcon: Icon(Icons.science_rounded)),
                    ...
                    TextFormField(
                      maxLines: 4,
                      decoration: const InputDecoration(labelText: "Clinical Results / Findings", alignLabelWithHint: true, prefixIcon: Icon(Icons.notes_rounded)),
                      onChanged: (v) => formData.results = v,
                      validator: evalName,
                    ),
                    const SizedBox(height: 16),
                    DropdownButtonFormField<String>(
                      decoration: const InputDecoration(labelText: "Process Status", prefixIcon: Icon(Icons.hourglass_empty_rounded)),
                      items: ['Pending', 'Processing', 'Verified'].map((v) => DropdownMenuItem(value: v, child: Text(v))).toList(),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        label: const Text("SUBMIT LAB REPORT"),
                      ),
                    ),
                  ],
                ),
        ...
        children: [
          ...
          Column(
            ...
            children: [
              ...
              const Text("Recording diagnostic findings and pathology results"),
            ],
          ),
          const Spacer(),
          OutlinedButton.icon(
  ...
  Widget _buildHistory() {
    return StreamBuilder<QuerySnapshot>(
      stream: authBloc.person.doc(formData.patientId).collection("Lab").orderBy('labDate', descending: true).snapshots(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator());
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return const Center(child: Text("No past lab records for this patient."));
        }
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final date = _getFormattedDate(data['labDate']);
            return Card(
              margin: const EdgeInsets.only(bottom: 12),
              child: ListTile(
                leading: CircleAvatar(backgroundColor: Colors.orange.shade50, child: const Icon(Icons.science_rounded, color: Colors.orange, size: 20)),
                title: Text("Test: ${data['lab']}"),
  ...
  void _confirmDelete(String docId) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text("Are you sure you want to remove this lab entry?"),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text("CANCEL")),
          TextButton(
            onPressed: () {
              authBloc.person.doc(formData.patientId).collection("Lab").doc(docId).delete();
